...
class CEMD:
    ...
    # def check_duplicate_in_one_system(self, dictionary={'elements': ['Sr', 'N']}):
    #     def is_in_mpr(struc, mpr_struc):
    #         exist = 0
    #         for ref in mpr_struc:
    #             dist = sm.StructureMatcher().fit(struc, ref)
    #             if dist:
    #                 exist = 1
    #                 break
    #         return exist
    #
    #     datalist = self.find(dictionary)
    #     structure_list = [CEMD.generate_structure_object(datalist[0])]
    #     for data in datalist:
    #         if not is_in_mpr(data, datalist):
    #             strcture_list.append(CEMD.generate_structure_object(data))
    #         else:
    #             db.test.delete_one({"_id": data["_id"]})
    ...
    def range_cleanup_duplicate(self, search_dictionary={'elements': ['Sr', 'N']}):
        ...
        cleaned = [CEMD.generate_structure_object(result[1])]

        deleted = []

        delete_count = 0
        for struc in result[2:]:
            for ref in cleaned:
        ...
        # for i in deleted
        #     self.posts.find_and_delete_one({"_id": struc["_id"]})

        print("deleted" + str(delete_count) + "data")
